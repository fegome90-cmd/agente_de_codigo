...
export class AgentUtils {
     ...
     */
    static calculateRoutingPriority(filesChanged, locChanged) {
        const agents = ['quality', 'pr_reviewer']; // Always active
        // Security triggers
        if (filesChanged > 0) {
    ...
    static calculateCost(tokens, model) {
        ...
        const rate = costPerToken[model] || 0.01 / 1000;
        return tokens * rate;
    }
    /**
     * Validate that an agent event has required fields
     */
    static validateAgentEvent(event) {
     ...
     */
    static createArtifactPath(agent, runId, type) {
     ...
     */
    static shouldActivateAgent(agent, files, patterns) {
        ...
        for (const file of files) {
            for (const pattern of patterns) {
                ...
                if (file.includes(pattern.pattern.replace('**/', '').replace('*.', ''))) {
                    return true;
                }
            }
        }
...
export const CONSTANTS = {
    ...
    KPI_TARGETS: {
        ...
        TARGET_COST_PER_REVIEW_USD: 0.50, // 50 cents
