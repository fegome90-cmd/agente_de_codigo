/**
 * TypeScript Socket Client for Observability Agent
...
export interface IPCMessage {
  id: string;
  type: 'task' | 'event' | 'heartbeat' | 'ping' | 'pong' | 'metric' | 'alert';
  agent: string;
  timestamp: string;
  data?: any;
}

export class ObservabilitySocketClient extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    this.isRunning = true;
    console.log(`[${this.agentName}] Starting observability client`);
   ...
   */
  stop(): void {
    console.log(`[${this.agentName}] Stopping observability client`);
   ...
   * Send metric data
   */
  sendMetric(metricName: string, value: number, labels?: Record<string, string | number>): boolean {
    return this.sendMessage({
      id: `metric-${Date.now()}`,
      type: 'metric',
      data: {
   ...
   * Send alert event
   */
  sendAlert(severity: 'info' | 'warning' | 'critical', message: string, details?: any): boolean {
    return this.sendMessage({
      id: `alert-${Date.now()}`,
      type: 'alert',
      data: {
   ...
   */
  sendHealthStatus(status: string, details?: any): boolean {
    return this.sendMessage({
      id: `health-${Date.now()}`,
      type: 'event',
      data: {
